...
obj.chrInputGCContents = containers.Map({1},{[]});
remove(obj.chrInputGCContents,1);
...
for i = 1:1:length(chrNames)
    ...
    if(bitget(BMObj1_Org.getFlag(1),1)==0)
        ...
        %%%----- For GC-correction -----%%
        if(gcCorrectionMethod == 2)
            BMObjCGcontents = BMObjSubset;
        end
        %%%
    ...
    %%-------- Paired-end reads --------%%
        ...
        %%%----- For GC-correction -----%%
        if(gcCorrectionMethod == 2)
            idx_Selected = sort([fow_idx_Selected;mate_idx_Selected]);
            BMObjCGcontents = getSubset(BMObjFiltered1, idx_Selected,'InMemory',inMemoryOpt);% Only Stored the two ends of the reads without constructing a fragment, can be used for RD calculation also if we deal seperately with each end.
            clear idx_Selected;
        end
        %%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% GC-contents %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    binsCounts = ceil(obj.chrLengths(targetChrIndex)/binSize);
    bpsCount = binsCounts*binSize;

    % Slow: Should be optimized 
    if(gcCorrectionMethod == 2)        
        refChr  = repmat(char(0),bpsCount,1);
        readStart = getStart(BMObjCGcontents);
        readCount = length(readStart);
        memFP = 100000;
        
        for j = 1:memFP:readCount
            FPStart = j;
            FPEnd = min(j+memFP,readCount);
            readSeq = char(getSequence(BMObjCGcontents,[FPStart:FPEnd]));
            m = 1;
            for k=FPStart:FPEnd
                readLength = length(readSeq(m,:)); 
                readStop  = readStart(k) + readLength -1;
                refChr(readStart(k):readStop) = readSeq(m,:);
                m = m +1;
            end
        end
        n = binSize;
        gcCount = arrayfun(@(k) sum(refChr(k:k+n-1)=='C' | refChr(k:k+n-1)=='G'),1:n:length(refChr));
        atCount = arrayfun(@(k) sum(refChr(k:k+n-1)=='A' | refChr(k:k+n-1)=='T'),1:n:length(refChr));
        gcRatio = gcCount./(gcCount+atCount);
        %%%
        obj.chrInputGCContents(targetChrIndex) = gcRatio;
        clear refChr readStart gcCount atCount gcRatio BMObjCGcontents;
    end    
    %%%
    %toc;
end
